...
#define MU_PACKCOLOUR(r, g, b, a) (((uint32_t)r) << 0) | ((g) << 8) | ((b) << 16) | ((a) << 24)
	...
	static const uint32_t ColourTable[ColourTableSize] = {
			...
			MU_PACKCOLOUR(0xa9, 0xda, 0x9a, 0xFF),
			MU_PACKCOLOUR(0x1f, 0xe6, 0x26, 0xFF),
			MU_PACKCOLOUR(0xd2, 0x3b, 0xe7, 0xFF),
			MU_PACKCOLOUR(0x72, 0xeb, 0x75, 0xFF),
			MU_PACKCOLOUR(0xdb, 0x55, 0x43, 0xFF),
			MU_PACKCOLOUR(0x38, 0xf7, 0xd0, 0xFF),
			MU_PACKCOLOUR(0x9a, 0xda, 0x49, 0xFF),
			MU_PACKCOLOUR(0x48, 0xa2, 0x20, 0xFF),
			MU_PACKCOLOUR(0xe6, 0xbb, 0x34, 0xFF),
			MU_PACKCOLOUR(0xe7, 0x3b, 0xd2, 0xFF)
	};
#undef MU_PACKCOLOUR
...
void VertexPosColour::UpdateIfNeededTriColours(MeshMod_MeshRenderable *mr){
	ASSERT(MeshMod_MeshHandleIsValid(mr->MMMesh));
...
void VertexPosColour::UpdateIfNeededFaceColours(MeshMod_MeshRenderable *mr){
	ASSERT(MeshMod_MeshHandleIsValid(mr->MMMesh));
